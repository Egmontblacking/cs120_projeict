...
with open(INPUT_FILENAME, "r") as f:
    ...
    for i in range(frame_num):
        ...
        # modulation (现在调制 DATA_FRAME_LEN + 8 个比特)
        frame_wave = np.zeros(len(frame_data_with_crc) * SAMPLES_PER_BIT)
        for j in range(len(frame_data_with_crc)):

            # OOK modulation
            if frame_data_with_crc[j] == "1":
                frame_wave[j * SAMPLES_PER_BIT : (j + 1) * SAMPLES_PER_BIT] = carrier[
                    j * SAMPLES_PER_BIT : (j + 1) * SAMPLES_PER_BIT
                ]
            else:
                frame_wave[j * SAMPLES_PER_BIT : (j + 1) * SAMPLES_PER_BIT] = 0

            # PSK modulation
            # frame_wave[j * SAMPLES_PER_BIT : (j + 1) * SAMPLES_PER_BIT] = carrier[j * SAMPLES_PER_BIT : (j + 1) * SAMPLES_PER_BIT] * (
